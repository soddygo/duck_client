...
fn ensure_duck_cli_binary() {
    ...
    println!("cargo:warning=Duck CLI binary not found: {}", binary_path.display());
    println!("cargo:warning=Auto-building duck-cli for target: {}", target_triple);
...
fn build_duck_cli(target_triple: &str) -> bool {
    println!("cargo:warning=Building duck-cli with: cargo build --release -p duck-cli --target {}", target_triple);
    
    let mut cmd = Command::new("cargo");
    cmd.args(&["build", "--release", "-p", "duck-cli", "--target", target_triple]);
    ...
    match cmd.status() {
        ...
        Ok(status) => {
            println!("cargo:warning=Failed to build duck-cli (exit code: {})", 
...
fn copy_binary_to_sidecar_location(target_triple: &str, binary_name: &str) {
    ...
    match std::fs::copy(&source_path, &dest_path) {
        Ok(_) => {
            println!("cargo:warning=Copied {} to {}", source_path.display(), dest_path.display());
        }
        Err(e) => {
