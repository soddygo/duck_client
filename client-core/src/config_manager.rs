        ...
        R: Send,
    {
        match self {
            ...
            DatabaseConnection::Database(_db) => {
                ...
                Err(crate::error::DuckError::custom("传统数据库连接暂不支持配置管理功能"))
            }
        }
    }
...
impl ConfigManager {
                ...
                db.read_with_retry(|conn| {
                    ...
                    let config_iter = stmt.query_map([], |row| {
                        ...
                        let value: Value = serde_json::from_str(&value_str)
                            .map_err(|e| duckdb::Error::InvalidParameterName(format!("JSON解析失败: {}", e)))?;
                        
                        let default_value = if let Some(default_str) = default_str {
                            Some(serde_json::from_str(&default_str)
                                .map_err(|e| duckdb::Error::InvalidParameterName(format!("默认值JSON解析失败: {}", e)))?)
                        } else {
                            None
                        };
                        
                        let config_type = ConfigType::from_str(&type_str)
                            .ok_or_else(|| duckdb::Error::InvalidParameterName(format!("无效的配置类型: {}", type_str)))?;
    ...
    pub async fn update_config(&self, key: &str, value: Value) -> Result<()> {
        ...
        if let Some(expected_type) = expected_type {
            if !self.validate_value_type(&value, &expected_type) {
                return Err(crate::DuckError::Custom(
                    format!("配置项 {} 的值类型不匹配，期望 {:?}，实际 {:?}", 
        ...
        for update in &updates {
            ...
            if let Some(config) = cache.get(&update.key) {
                ...
                if update.validate && !self.validate_value_type(&update.value, &config.config_type) {
                    return Err(crate::DuckError::Custom(
                        format!("配置项 {} 的值类型不匹配", update.key)
                    ).into());
                }
            } else {
                return Err(crate::DuckError::Custom(
                    format!("配置项 {} 不存在", update.key)
                ).into());
            }
        }
    ...
    pub async fn get_auto_backup_config(&self) -> Result<AutoBackupConfig> {
        ...
        let retention_days = self.get_integer("auto_backup_retention_days").await?.unwrap_or(7) as i32;
        let backup_dir = self.get_string("auto_backup_directory").await?.unwrap_or("./backups".to_string());
        
        let last_backup_time = if let Some(time_str) = self.get_string("auto_backup_last_time").await? {
            ...
            let tasks = stmt.query_map([], |row| {
                ...
                Ok(AutoUpgradeTask {
                    ...
                    schedule_time: chrono::DateTime::parse_from_rfc3339(&schedule_time_str)
                        .map_err(|_| duckdb::Error::InvalidColumnType(0, "schedule_time".to_string(), duckdb::types::Type::Text))?
                        .with_timezone(&chrono::Utc),
                    upgrade_type: row.get("upgrade_type")?,
                    target_version: if target_version.is_empty() { None } else { Some(target_version) },
                    status: row.get("status")?,
                    progress: if progress_str.is_empty() { None } else { progress_str.parse().ok() },
                    error_message: if error_msg.is_empty() { None } else { Some(error_msg) },
                    created_at: chrono::DateTime::parse_from_rfc3339(&created_at_str)
                        .map_err(|_| duckdb::Error::InvalidColumnType(0, "created_at".to_string(), duckdb::types::Type::Text))?
                        .with_timezone(&chrono::Utc),
                    updated_at: chrono::DateTime::parse_from_rfc3339(&updated_at_str)
                        .map_err(|_| duckdb::Error::InvalidColumnType(0, "updated_at".to_string(), duckdb::types::Type::Text))?
                        .with_timezone(&chrono::Utc),
                })
            })?;
