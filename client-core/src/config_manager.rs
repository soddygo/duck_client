...
impl<'a> ConfigManager<'a> {
    ...
    pub async fn get_auto_backup_config(&self) -> Result<AutoBackupConfig> {
        // 获取各个配置项
        let enabled = self.db.get_config("auto_backup.enabled").await?
            .unwrap_or_else(|| "true".to_string())
            .parse::<bool>()
            .unwrap_or(true);
        
        let cron_expression = self.db.get_config("auto_backup.cron_expression").await?
            .unwrap_or_else(|| "0 2 * * *".to_string());
        
        let last_backup_at = self.db.get_config("auto_backup.last_backup_at").await?
            .and_then(|s| DateTime::parse_from_rfc3339(&s).ok().map(|dt| dt.with_timezone(&Utc)));
        
        let consecutive_failures = self.db.get_config("auto_backup.consecutive_failures").await?
            .unwrap_or_else(|| "0".to_string())
            .parse::<i32>()
            .unwrap_or(0);
        
        let max_failures = self.db.get_config("auto_backup.max_failures").await?
    ...
    pub async fn set_auto_backup_enabled(&self, enabled: bool) -> Result<()> {
        self.db.set_config("auto_backup.enabled", &enabled.to_string()).await?;
    ...
    pub async fn set_auto_backup_cron(&self, cron_expression: &str) -> Result<()> {
        self.db.set_config("auto_backup.cron_expression", cron_expression).await?;
        tracing::info!("自动备份cron表达式已更新: {}", cron_expression);
        Ok(())
    }

    /// 记录备份执行时间
    pub async fn update_last_backup_time(&self, backup_time: DateTime<Utc>, success: bool) -> Result<()> {
        self.db.set_config("auto_backup.last_backup_at", &backup_time.to_rfc3339()).await?;
        
        if success {
            self.db.set_config("auto_backup.consecutive_failures", "0").await?;
        } else {
            let current_failures = self.db.get_config("auto_backup.consecutive_failures").await?
                .unwrap_or_else(|| "0".to_string())
                .parse::<i32>()
                .unwrap_or(0);
            self.db.set_config("auto_backup.consecutive_failures", &(current_failures + 1).to_string()).await?;
        }
    ...
    pub async fn create_auto_upgrade_task(&self, task: &AutoUpgradeTask) -> Result<String> {
        ...
        let task_json = serde_json::to_string(task).map_err(|e| 
            crate::error::DuckError::custom(format!("序列化任务失败: {}", e)))?;
    ...
    /// 更新升级任务状态
    pub async fn update_upgrade_task_status(&self, task_id: &str, status: &str, progress: Option<i32>, error_message: Option<&str>) -> Result<()> {
        let key = format!("auto_upgrade_task.{}", task_id);
    ...
    pub async fn get_pending_upgrade_tasks(&self) -> Result<Vec<(String, AutoUpgradeTask)>> {
        ...
        tracing::warn!("get_pending_upgrade_tasks: 当前实现不支持查询所有任务，需要扩展Database接口");
        Ok(vec![])
    }
    ...
    pub async fn get_auto_backup_status_info(&self) -> Result<String> {
        ...
        let status_info = format!(
            ...
            config.last_backup_at.map(|t| t.format("%Y-%m-%d %H:%M:%S UTC").to_string())
    ...
    async fn validate_backup_id(&self, backup_id: i32) -> Result<()> {
        // 检查backup_id是否为正数
        if backup_id <= 0 {
            return Err(crate::error::DuckError::custom(
                format!("无效的backup_id: {}，必须为正数", backup_id)
            ));
        }
        ...
        let backup_record = match self.db.get_backup_by_id(backup_id as i64).await? {
            ...
            None => {
                return Err(crate::error::DuckError::custom(
                    format!("备份记录不存在: backup_id = {}", backup_id)
                ));
            }
        };
        ...
        if backup_record.status != crate::database::BackupStatus::Completed {
            return Err(crate::error::DuckError::custom(
                format!("备份记录状态不正确: backup_id = {}，状态 = {:?}，只有完成状态的备份才能使用", 
        ...
        if !backup_file_path.exists() {
            return Err(crate::error::DuckError::custom(
                format!("备份文件不存在: backup_id = {}，文件路径 = {}。\n可能文件已被移动或删除，请检查备份文件的完整性", 
                       backup_id, backup_record.file_path)
            ));
        }
        
                 // 检查文件是否为.zip格式（额外的安全检查）
         if !backup_record.file_path.to_lowercase().ends_with(crate::constants::file_format::ZIP_EXTENSION) {
             tracing::warn!("备份文件 {} 不是.zip格式，可能存在问题", backup_record.file_path);
         }
         
         // 简单验证ZIP文件头（验证文件确实是ZIP格式）
         if let Err(e) = Self::verify_zip_file_header(&backup_file_path) {
             return Err(crate::error::DuckError::custom(
                 format!("备份文件格式验证失败: backup_id = {}，文件路径 = {}，错误: {}", 
         ...
         if let Ok(metadata) = backup_file_path.metadata() {
             ...
             if file_size == 0 {
                 return Err(crate::error::DuckError::custom(
                     format!("备份文件为空: backup_id = {}，文件路径 = {}", 
                            backup_id, backup_record.file_path)
                 ));
             }
             
             // 检查文件大小是否合理（至少应该有一些内容）
             if file_size < crate::constants::backup::MIN_ZIP_FILE_SIZE { // 至少100字节，一个有效的zip文件应该比这大
                 tracing::warn!("备份文件 {} 大小过小 ({} bytes)，可能存在问题", 
                               backup_record.file_path, file_size);
             }
             
             tracing::debug!("备份文件检查通过: backup_id = {}，文件路径 = {}，文件大小 = {} bytes", 
                            backup_id, backup_record.file_path, file_size);
         }
        
        tracing::info!("备份ID {} 验证通过，文件路径: {}", backup_id, backup_record.file_path);
        Ok(())
    }
    ...
    fn verify_zip_file_header(file_path: &std::path::Path) -> Result<()> {
        ...
        let mut file = File::open(file_path)
            .map_err(|e| crate::error::DuckError::custom(
                format!("无法打开文件进行验证: {}", e)
            ))?;
            ...
            Ok(_) => {
                ...
                if buffer[0..2] == file_format::ZIP_MAGIC_PK_PREFIX {
                    ...
                    if buffer == file_format::ZIP_MAGIC_LOCAL_HEADER {
                        ...
                        tracing::debug!("ZIP文件头验证通过: PK格式但子类型未知 ({:#04x} {:#04x})", buffer[2], buffer[3]);
                        Ok(())
                    }
                } else {
                    Err(crate::error::DuckError::custom(
                        format!("文件不是有效的ZIP格式，文件头: {:#04x} {:#04x} {:#04x} {:#04x}", 
