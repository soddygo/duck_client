...
impl DockerManager {
    ...
    pub async fn restart_service(&self, service_name: &str) -> Result<()> {
        ...
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(DuckError::Docker(format!("停止服务 {} 失败: {}", service_name, stderr)));
        }
        ...
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(DuckError::Docker(format!("启动服务 {} 失败: {}", service_name, stderr)));
        }
    ...
    pub async fn wait_for_services_started_after_deploy(&self) -> Result<()> {
        self.verify_services_started(Some(timeout::DEPLOY_START_TIMEOUT)).await
    }
    ...
    async fn verify_services_started(&self, custom_timeout: Option<u64>) -> Result<()> {
        ...
        // 使用统一的常量配置
        let max_wait_time = Duration::from_secs(custom_timeout.unwrap_or(timeout::SERVICE_START_TIMEOUT));
            ...
            match self.get_services_status().await {
                Ok(services) => {
                    ...
                    for service in &services {
                        match service.status {
                            ...
                            ServiceStatus::Stopped => {
                                // 检查这是否是一次性任务服务
                                if self.is_oneshot_service(&service.name).await.unwrap_or(false) {
                                    tracing::debug!("服务 {} 是一次性任务，已正常退出", service.name);
                                } else {
                    ...
                    if attempt == max_attempts {
                        ...
                        if !failed_services.is_empty() {
                            error_msg.push_str(&format!("启动失败的服务: {}", failed_services.join(", ")));
                        }
                        if !pending_services.is_empty() {
                            if !error_msg.is_empty() {
                                error_msg.push_str("; ");
                            }
                            error_msg.push_str(&format!("启动超时的服务: {}", pending_services.join(", ")));
                        }
