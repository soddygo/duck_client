...
impl DatabaseManager {
            ...
            match operation(&conn) {
                ...
                Err(e) => {
                    ...
                    if Self::is_retryable_error(&error_msg) && retry_count < MAX_RETRIES {
                        ...
                        warn!("读操作失败，{}ms后重试 ({}/{}): {}", 
                              delay.as_millis(), retry_count, MAX_RETRIES, error_msg);
    ...
    pub async fn initialize_schema(&self) -> Result<()> {
        ...
        self.write_with_retry(|conn| {
            ...
            for statement in statements {
                ...
                // 检查是否只包含注释
                let is_only_comments = trimmed.lines()
                    .map(|line| line.trim())
                ...
                debug!("执行SQL语句: {}", if trimmed.len() > 100 { 
                    // 安全地截取字符串，避免切断多字节字符
                    let mut end = 100;
    ...
    pub async fn health_check(&self) -> Result<HealthStatus> {
        // 检查读连接
        let read_result = self.read_with_retry(|conn| {
        ...
        // 检查写连接
        let write_result = self.write_with_retry(|conn| {
            conn.query_row("SELECT 1", [], |row| {
                let value: i32 = row.get(0)?;
                Ok(value)
            })
...
mod tests {
    ...
    async fn test_write_operations() {
        ...
        // 测试写操作
        let result = manager.write_with_retry(|conn| {
        ...
        // 验证数据是否写入
        let value = manager.read_with_retry(|conn| {
    ...
    async fn test_batch_write_operations() {
        ...
        // 测试批量写操作
        let result = manager.batch_write_with_retry(|conn| {
            conn.execute("CREATE TABLE IF NOT EXISTS batch_test (id INTEGER, value TEXT)", [])?;
        ...
        // 验证所有数据都已写入
        let count = manager.read_with_retry(|conn| {
    ...
    async fn test_file_database_concurrent_operations() {
        ...
        // 创建测试表
        manager.write_with_retry(|conn| {
            conn.execute("CREATE TABLE IF NOT EXISTS concurrent_test (id INTEGER, value TEXT)", [])?;
            Ok(())
        }).await.unwrap();
        ...
        for i in 0..5 {
            ...
            let handle = tokio::spawn(async move {
                manager.write_with_retry(|conn| {
                    conn.execute("INSERT INTO concurrent_test (id, value) VALUES (?, ?)", 
    ...
    async fn test_debug_sql_initialization() {
        ...
        if init_result.is_ok() {
            ...
            if result.is_ok() {
                let app_config_exists_after = manager.debug_table_exists("app_config").await.unwrap();
    ...
    async fn test_debug_individual_sql_statements() {
        ...
        for (i, stmt) in statements.iter().enumerate() {
            ...
            println!("执行语句 {}: {}", i + 1, if trimmed.len() > 100 { 
                // 安全地截取字符串，避免切断多字节字符
                let mut end = 100;
                while end > 0 && !trimmed.is_char_boundary(end) {
                    end -= 1;
