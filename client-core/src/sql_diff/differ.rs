...
pub fn generate_mysql_diff(
    ...
    diff_sql.push(format!("-- 生成时间: {}", chrono::Utc::now().format("%Y-%m-%d %H:%M:%S UTC")));
    ...
    // 如果只有注释头，说明没有实际差异
    let meaningful_lines: Vec<&str> = result.lines()
...
/// 生成表差异SQL
pub fn generate_table_diff(old_table: &TableDefinition, new_table: &TableDefinition) -> Vec<String> {
...
fn generate_column_diffs(old_table: &TableDefinition, new_table: &TableDefinition) -> Vec<String> {
    ...
    // 创建列名到列定义的映射
    let old_columns: HashMap<String, &TableColumn> = old_table.columns.iter().map(|c| (c.name.clone(), c)).collect();
    let new_columns: HashMap<String, &TableColumn> = new_table.columns.iter().map(|c| (c.name.clone(), c)).collect();
    
    // 检查新增的列
    for (col_name, col_def) in &new_columns {
        if !old_columns.contains_key(col_name) {
            diffs.push(format!("ALTER TABLE `{}` ADD COLUMN {};", table_name, generate_column_sql(col_def)));
        }
    }
    ...
    for col_name in old_columns.keys() {
        if !new_columns.contains_key(col_name) {
            diffs.push(format!("ALTER TABLE `{table_name}` DROP COLUMN `{col_name}`;"));
        }
    }
    ...
    for (col_name, new_col) in &new_columns {
        if let Some(old_col) = old_columns.get(col_name) {
            if old_col != new_col {
                diffs.push(format!("ALTER TABLE `{}` MODIFY COLUMN {};", table_name, generate_column_sql(new_col)));
            }
        }
    }
...
fn generate_index_diffs(old_table: &TableDefinition, new_table: &TableDefinition) -> Vec<String> {
    ...
    // 创建索引名到索引定义的映射
    let old_indexes: HashMap<String, &TableIndex> = old_table.indexes.iter().map(|i| (i.name.clone(), i)).collect();
    let new_indexes: HashMap<String, &TableIndex> = new_table.indexes.iter().map(|i| (i.name.clone(), i)).collect();
    
    // 检查新增的索引
    for (idx_name, idx_def) in &new_indexes {
        if !old_indexes.contains_key(idx_name) {
            if idx_def.is_primary {
                diffs.push(format!("ALTER TABLE `{}` ADD PRIMARY KEY ({});", 
                    table_name, 
                    idx_def.columns.iter().map(|c| format!("`{c}`")).collect::<Vec<_>>().join(", ")));
            } else if idx_def.is_unique {
                diffs.push(format!("ALTER TABLE `{}` ADD UNIQUE KEY `{}` ({});", 
                    table_name, 
                    idx_name,
                    idx_def.columns.iter().map(|c| format!("`{c}`")).collect::<Vec<_>>().join(", ")));
            } else {
                diffs.push(format!("ALTER TABLE `{}` ADD KEY `{}` ({});", 
                    table_name, 
                    idx_name,
                    idx_def.columns.iter().map(|c| format!("`{c}`")).collect::<Vec<_>>().join(", ")));
            }
        ...
        if let Some(old_idx) = old_indexes.get(idx_name) {
            if old_idx != new_idx {
                ...
                if new_idx.is_primary {
                    diffs.push(format!("ALTER TABLE `{}` ADD PRIMARY KEY ({});", 
                        table_name, 
                        new_idx.columns.iter().map(|c| format!("`{c}`")).collect::<Vec<_>>().join(", ")));
