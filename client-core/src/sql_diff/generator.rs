...
pub fn generate_schema_diff(
    ...
    match from_sql {
        ...
        Some(from_content) => {
            info!("开始生成版本 {} 到 {} 的SQL差异", from_version.unwrap_or("unknown"), to_version);
            
            // 如果内容完全相同，返回空差异
            if from_content.trim() == to_sql.trim() {
                info!("版本内容完全相同，无需生成差异");
                return Ok((String::new(), format!("版本 {} 到 {}: 无变化", from_version.unwrap_or("unknown"), to_version)));
            }
            ...
            let description = if diff_sql.trim().is_empty() {
                format!("版本 {} 到 {}: 仅有注释或格式变化，无实际架构差异", from_version.unwrap_or("unknown"), to_version)
            } else {
                let lines_count = diff_sql.lines().filter(|line| !line.trim().is_empty() && !line.trim().starts_with("--")).count();
                ...
                format!("版本 {} 到 {}: {} - 生成 {} 行可执行的差异SQL", 
                    from_version.unwrap_or("unknown"), to_version, change_summary, lines_count)
            };
...
pub fn generate_index_sql(index: &TableIndex) -> String {
    if index.is_primary {
        format!("PRIMARY KEY ({})", index.columns.iter().map(|c| format!("`{c}`")).collect::<Vec<_>>().join(", "))
    } else if index.is_unique {
        format!("UNIQUE KEY `{}` ({})", index.name, index.columns.iter().map(|c| format!("`{c}`")).collect::<Vec<_>>().join(", "))
    } else {
        format!("KEY `{}` ({})", index.name, index.columns.iter().map(|c| format!("`{c}`")).collect::<Vec<_>>().join(", "))
    }
} 