...
use crate::commands::{docker_service, backup, update};
...
pub async fn schedule_delayed_deploy(app: &mut CliApp, time: u32, unit: &str) -> Result<()> {
    ...
    let delay_seconds = match unit.to_lowercase().as_str() {
        ...
        _ => {
            ...
            return Err(client_core::error::DuckError::custom(
                format!("不支持的时间单位: {}，支持的单位: hours, minutes, days", unit)
            ));
        }
    };
    ...
    info!("   计划执行时间: {}", scheduled_at.format("%Y-%m-%d %H:%M:%S UTC"));
    
    info!("安排延迟执行自动升级部署: {} {}，任务ID: {}", time, unit, task_id);
    
    // 更新任务状态为进行中
    {
        let config_manager = client_core::config_manager::ConfigManager::new(&app.database);
        config_manager.update_upgrade_task_status(&task_id, "in_progress", Some(0), None).await?;
    }
    ...
    match run_auto_upgrade_deploy(app).await {
        Ok(_) => {
            let config_manager = client_core::config_manager::ConfigManager::new(&app.database);
            config_manager.update_upgrade_task_status(&task_id, "completed", Some(100), None).await?;
            info!("✅ 延迟升级部署任务完成");
        }
        Err(e) => {
            let config_manager = client_core::config_manager::ConfigManager::new(&app.database);
            config_manager.update_upgrade_task_status(&task_id, "failed", None, Some(&e.to_string())).await?;
            error!("延迟升级部署任务失败: {}", e);
            return Err(e);
        }
    }
        ...
        Ok(tasks) => {
            if tasks.is_empty() {
                ...
                for (task_id, task) in tasks {
                    ...
                    info!("     状态: {}", task.status);
                    info!("     计划执行时间: {}", task.scheduled_at.format("%Y-%m-%d %H:%M:%S UTC"));
