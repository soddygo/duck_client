...
use client_core::{backup::{BackupOptions, RestoreOptions}, database::BackupType, error::Result};
...
pub async fn run_backup(app: &CliApp) -> Result<()> {
            ...
            if status.overall_status != ServiceStatus::AllStopped {
                ...
                for container in status.containers.iter() {
                    if container.status.is_healthy() {
                        info!("   - {} (状态: {})", container.name, container.status.display_name());
                    }
                }
                ...
                info!("   或者");
                info!("   cd {} && docker-compose down", compose_path.parent().unwrap_or(std::path::Path::new(".")).display());
    ...
    if has_app {
        info!("     ✅ 应用目录: {} (Java工程和前端资源)", app_dir.display());
    }
...
pub async fn run_list_backups(app: &CliApp) -> Result<()> {
    ...
    if invalid_backups > 0 {
        ...
        info!("💡 建议:");
        info!("   - 检查备份目录设置: {}", app.config.get_backup_dir().display());
...
pub async fn run_rollback(app: &CliApp, backup_id: i64, force: bool) -> Result<()> {
    ...
    app.backup_manager.restore_from_backup(backup_id, options).await?;
    info!("✅ 回滚完成");
    Ok(())
} 