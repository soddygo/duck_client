...
pub async fn check_for_updates(repo: &GitHubRepo) -> Result<VersionInfo> {
    ...
    let is_update_available = compare_versions(&current_version, &latest_version) == std::cmp::Ordering::Less;
...
fn find_platform_asset(assets: &[GitHubAsset]) -> Option<String> {
    ...
    for asset in assets {
        ...
        if platform_patterns.iter().any(|pattern| name_lower.contains(pattern)) {
            // 优先选择可执行文件
            if name_lower.contains("duck-cli") || name_lower.ends_with(".exe") || name_lower.ends_with(".tar.gz") {
...
/// 检查版本并决定是否需要安装
pub async fn should_install(repo: &GitHubRepo, target_version: Option<&str>, force: bool) -> Result<(String, String)> {
    let current_version = get_current_version();
...
/// 安装下载的文件
async fn install_downloaded_file(download_path: &PathBuf, current_exe: &PathBuf, version: &str) -> Result<()> {
    let download_name = download_path.file_name()
        .and_then(|n| n.to_str())
        .unwrap_or("");
...
/// 从压缩包安装
async fn install_from_archive(archive_path: &PathBuf, current_exe: &PathBuf, _version: &str) -> Result<()> {
    ...
    match command {
        ...
        CheckUpdateCommand::Install { version, force } => {
            info!("🚀 开始安装 Duck CLI...");
                        ...
                        // 检查是否需要安装
            let (current_version, target_version) = match should_install(&repo, version.as_deref(), force).await {
            ...
            info!("准备从版本 {} 更新到版本 {}", current_version, target_version);
...
async fn get_version_download_url(repo: &GitHubRepo, version: &str) -> Result<String> {
    ...
    version_info.download_url.ok_or_else(|| {
        anyhow::anyhow!("未找到版本 {} 适合当前平台的下载链接", version)
    })
} 