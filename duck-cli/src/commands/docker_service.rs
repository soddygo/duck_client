...
use crate::docker_service::{DockerService, DockerServiceManager, ContainerStatus};
...
pub async fn deploy_docker_services(app: &CliApp) -> Result<()> {
    ...
    info!("工作目录: {}", docker_service_manager.get_work_dir().display());
    ...
    match docker_service_manager.deploy_services().await {
        ...
        Err(e) => {
            ...
            return Err(client_core::DuckError::custom(format!("Docker 服务部署失败: {:?}", e)));
        }
    }
...
pub async fn restart_container(app: &CliApp, container_name: &str) -> Result<()> {
    ...
    match docker_service_manager.restart_container(container_name).await {
        Ok(_) => {
...
pub async fn check_docker_services_status(app: &CliApp) -> Result<()> {
    ...
    match docker_service_manager.get_service_status().await {
        Ok(report) => {
            info!("=== Docker 服务状态报告 ===");
            info!("检查时间: {}", report.check_time.format("%Y-%m-%d %H:%M:%S UTC"));
            info!("整体状态: {}", report.overall_status.display_name());
            info!("运行统计: {}/{} 个容器正在运行", report.running_count, report.total_count);
        ...
        Err(e) => {
            error!("❌ 获取服务状态失败: {:?}", e);
            return Err(client_core::DuckError::custom(format!("获取服务状态失败: {:?}", e)));
        }
    }
...
pub async fn show_architecture_info(_app: &CliApp) -> Result<()> {
    ...
    info!("  • 镜像后缀: {}", crate::docker_service::get_architecture_suffix(arch));
