...
async fn check_update_server_available(_app: &CliApp) -> Result<UpdateInfo> {
    ...
    Ok(UpdateInfo {
        ...
        has_update,
        download_url: Some(format!("{}/downloads/duck-cli-{}-{}", base_url, current_version, _platform)),
...
pub async fn run_upgrade(app: &mut CliApp, full: bool, force: bool) -> Result<()> {
    ...
    match app.api_client.check_docker_version(current_version).await {
        Ok(version_info) => {
            ...
            let download_path = app.config.get_version_download_file_path(target_version, download_type, client_core::constants::upgrade::DOCKER_SERVICE_PACKAGE);
            ...
            // 判断是否需要下载
            let should_download = is_first_time || force || version_info.has_update || full || !file_exists;
            ...
            if should_download {
                ...
                // 确保下载目录存在
                if let Err(e) = app.config.ensure_version_download_dir(target_version, download_type) {
                    error!("❌ 创建下载目录失败: {}", e);
                    return Err(e);
                }
        ...
        Err(e) => {
            ...
            let download_path = app.config.get_version_download_file_path(fallback_version, download_type, client_core::constants::upgrade::DOCKER_SERVICE_PACKAGE);
            let file_exists = download_path.exists();
