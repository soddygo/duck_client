    ...
    fn apply_env_based_user_mapping(&self) -> DockerServiceResult<()> {
        ...
        if let Some(compose_file) = self.find_compose_file() {
            let content = fs::read_to_string(&compose_file).map_err(|e| {
                DockerServiceError::FileSystem(format!("读取docker-compose.yml失败: {e}"))
            })?;
            ...
            fs::write(&compose_file, modified_content).map_err(|e| {
                DockerServiceError::FileSystem(format!("写入docker-compose.yml失败: {e}"))
            })?;
            ...
            Err(DockerServiceError::Configuration(
                "未找到docker-compose.yml文件".to_string(),
            ))
        }
    }
...
",
        ...
        if let Some(compose_file) = self.find_compose_file() {
            ...
            if !data_dir.exists() {
                fs::create_dir_all(&data_dir).map_err(|e| {
                    DockerServiceError::FileSystem(format!("创建基础data目录失败: {e}"))
                })?;
                info!("✅ 已创建基础data目录");
            }
    ...
    fn ensure_bind_mount_parent_directory(&self, bind_mount_path: &str) -> DockerServiceResult<()> {
        // 移除 "./" 前缀，获取相对路径
        let relative_path = bind_mount_path
            .strip_prefix("./")
            .unwrap_or(bind_mount_path);
