...
impl HealthChecker {
    ...
    fn log_progress(&self, report: &HealthReport, elapsed: Duration) {
        let running_containers = report.containers.iter()
            .filter(|c| c.status.is_healthy())
            .map(|c| &c.name)
            .collect::<Vec<_>>();
            
        let starting_containers = report.get_starting_containers()
            .iter()
            .map(|c| &c.name)
            .collect::<Vec<_>>();
    ...
    /// 检查特定容器的状态
    pub async fn check_container_status(&self, container_name: &str) -> DockerServiceResult<ContainerInfo> {
        let report = self.check_health().await?;
        ...
        Err(DockerServiceError::ServiceManagement(
            format!("未找到容器: {}", container_name)
        ))
    }
    ...
    pub async fn get_status_summary(&self) -> DockerServiceResult<String> {
        ...
        if !failed_containers.is_empty() {
            let failed_names: Vec<&str> = failed_containers.iter()
                .map(|c| c.name.as_str())
                .collect();
            summary.push_str(&format!("\n失败容器: {:?}", failed_names));
        }
...
mod tests {
    ...
    fn test_container_status_from_str() {
        ...
        assert_eq!(ContainerStatus::from_str("starting"), ContainerStatus::Starting);
        assert_eq!(ContainerStatus::from_str("unknown"), ContainerStatus::Unknown);
    }
