...
impl ImageInfo {
    /// 从文件路径推断镜像信息
    pub fn from_file_path(file_path: PathBuf, architecture: Architecture) -> DockerServiceResult<Self> {
        let file_name = file_path.file_name()
            .and_then(|n| n.to_str())
            .ok_or_else(|| DockerServiceError::ImageLoading("无效的文件名".to_string()))?;
        
        // 推断镜像类型
        let image_type = if file_name.contains("mysql") || file_name.contains("redis") 
            || file_name.contains("nginx") || file_name.contains("postgres") {
...
impl ImageLoader {
    ...
    pub fn scan_architecture_images(&self) -> DockerServiceResult<Vec<ImageInfo>> {
        if !self.images_dir.exists() {
            return Err(DockerServiceError::ImageLoading(
                format!("镜像目录不存在: {}", self.images_dir.display())
            ));
        }
                ...
                if let Some(file_name) = path.file_name().and_then(|n| n.to_str()) {
                    if file_name.ends_with(&arch_suffix) {
                        match ImageInfo::from_file_path(path.clone(), self.architecture) {
                            Ok(image_info) => {
                                info!("发现镜像文件: {} ({})", file_name, format_file_size(image_info.file_size));
        ...
        if images.is_empty() {
            return Err(DockerServiceError::ImageLoading(
                format!("未找到 {} 架构的镜像文件", self.architecture.as_str())
            ));
        }
        
        info!("共发现 {} 个 {} 架构的镜像文件", images.len(), self.architecture.as_str());
        Ok(images)
    }
    ...
    pub async fn load_all_images(&self) -> DockerServiceResult<LoadResult> {
        ...
        for (index, image) in images.iter().enumerate() {
            ...
            let file_name = image.file_path.file_name()
                .and_then(|n| n.to_str())
                .unwrap_or("unknown");
            
            info!("{} 加载镜像: {} ({})", progress, file_name, format_file_size(image.file_size));
        ...
        info!("镜像加载完成: 成功 {}, 失败 {}", result.success_count, result.failure_count);
        Ok(result)
    }
    ...
    pub async fn setup_image_tags(&self) -> DockerServiceResult<TagResult> {
        ...
        for image in images {
            if image.original_tag != image.target_tag {
                let original_with_latest = format!("{}:latest-{}", image.target_tag, self.architecture.as_str());
                let target_with_latest = format!("{}:latest", image.target_tag);
                
                info!("设置标签: {} -> {}", original_with_latest, target_with_latest);
                
                // 使用 docker tag 命令设置标签
                match self.tag_image(&original_with_latest, &target_with_latest).await {
                    Ok(_) => {
        ...
        info!("标签设置完成: 成功 {}, 失败 {}", result.success_count, result.failure_count);
        Ok(result)
    }
    ...
    async fn tag_image(&self, source_tag: &str, target_tag: &str) -> DockerServiceResult<()> {
        ...
        if !output.status.success() {
            ...
            return Err(DockerServiceError::DockerCommand(
                format!("设置镜像标签失败: {}", stderr)
            ));
        }
