        ...
        match self {
            ServiceFilter::NameContains(keywords) => {
                ...
                keywords.iter().any(|keyword| {
                    container.names.to_lowercase().contains(&keyword.to_lowercase())
                })
            }
            ...
            ServiceFilter::ServiceConfigs(configs) => {
                ...
                configs.iter().any(|config| {
                    container.names.to_lowercase().contains(&config.name.to_lowercase())
                })
            }
    ...
    match ducker::docker::util::new_local_docker_connection(
        ...
        Ok(docker) => {
            match DockerContainer::list(&docker).await {
                Ok(containers) => {
                    let filtered_containers: Vec<_> = containers.iter()
                        .filter(|c| filter.matches(c))
                        .collect();
                    
                    // 智能计算"正常运行"的服务数量
                    let running_count = filtered_containers.iter()
                        .filter(|container| {
                    ...
                    match filter {
                        ServiceFilter::All => {
                            info!("发现 {} 个正在运行的容器（总共 {} 个）", running_count, total_filtered);
                        }
                        ServiceFilter::NameContains(keywords) => {
                            info!("匹配关键字 {:?} 的容器: {} 个运行中（总共 {} 个）", keywords, running_count, total_filtered);
                        }
                        ServiceFilter::ContainerIds(ids) => {
                            info!("匹配ID {:?} 的容器: {} 个运行中（总共 {} 个）", ids, running_count, total_filtered);
                        }
                        ServiceFilter::ServiceConfigs(configs) => {
                            let service_names: Vec<_> = configs.iter().map(|c| &c.name).collect();
                            info!("匹配服务配置 {:?} 的容器: {} 个运行中（总共 {} 个）", service_names, running_count, total_filtered);
                        }
                    }
                ...
                Err(e) => {
                    error!("获取容器列表失败: {}", e);
                    Err(client_core::error::DuckError::docker_service(
                        format!("获取容器列表失败: {}", e)
                    ))
                }
            }
...
pub async fn wait_for_services_stopped(filter: &ServiceFilter, timeout_secs: u64) -> Result<bool> {
    ...
    info!("开始等待服务停止，过滤条件: {:?}，超时: {} 秒", filter, timeout_secs);
...
pub async fn wait_for_services_started(filter: &ServiceFilter, timeout_secs: u64) -> Result<bool> {
    ...
    info!("开始等待服务启动，过滤条件: {:?}，超时: {} 秒", filter, timeout_secs);
...
pub async fn parse_service_configs_from_compose(compose_file_path: &Path) -> Result<Vec<ServiceConfig>> {
    if !compose_file_path.exists() {
        warn!("docker-compose.yml 文件不存在: {}", compose_file_path.display());
        return Ok(vec![]);
    }
    ...
    match fs::read_to_string(compose_file_path) {
        Ok(content) => {
            match serde_yaml::from_str::<Value>(&content) {
                Ok(yaml) => {
                        ...
                        if let Some(services_map) = services.as_mapping() {
                            for (key, value) in services_map {
                                if let Some(service_name) = key.as_str() {
                                    let restart_policy = if let Some(service_config) = value.as_mapping() {
                    ...
                    info!("从 {} 解析到 {} 个服务配置:", compose_file_path.display(), service_configs.len());
                ...
                Err(e) => {
                    ...
                    Err(client_core::error::DuckError::custom(
                        format!("解析docker-compose.yml失败: {}", e)
                    ))
                }
            }
        }
        Err(e) => {
            ...
            Err(client_core::error::DuckError::custom(
                format!("读取docker-compose.yml文件失败: {}", e)
            ))
        }
    }
}
...
pub async fn parse_service_names_from_compose(compose_file_path: &Path) -> Result<Vec<String>> {
    ...
    Ok(service_configs.into_iter().map(|config| config.name).collect())
}
...
/// 便捷函数：等待compose服务停止
pub async fn wait_for_compose_services_stopped(compose_file_path: &Path, timeout_secs: u64) -> Result<bool> {
    let filter = create_compose_filter(compose_file_path).await?;
    wait_for_services_stopped(&filter, timeout_secs).await
}

/// 便捷函数：等待compose服务启动
pub async fn wait_for_compose_services_started(compose_file_path: &Path, timeout_secs: u64) -> Result<bool> {
    let filter = create_compose_filter(compose_file_path).await?;
