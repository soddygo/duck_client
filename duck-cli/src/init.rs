...
pub async fn run_init(force: bool) -> Result<()> {
    ...
    // 检查是否已经初始化过
    if !force && (std::path::Path::new("config.toml").exists() || std::path::Path::new("history.db").exists()) {
    ...
    info!("      - {}         (备份存储目录)", config.backup.storage_dir);
    ...
    info!("   • 运行 'duck-cli auto-upgrade-deploy delay-time-deploy 2 --unit hours' 延时2小时后自动部署");
    info!("");
    info!("💡 提示:");
    info!("   - 配置文件: config.toml (可手动编辑修改配置)");
    info!("   - 数据库文件: history.db (存储操作历史和备份记录)");
