        ...
        for line in meminfo.lines() {
            if line.starts_with("MemTotal:") {
                total = line.split_whitespace()
                    ...
                    .unwrap_or(8388608) * 1024; // kB to bytes
            } else if line.starts_with("MemAvailable:") {
                available = line.split_whitespace()
                    .nth(1)
                    .and_then(|s| s.parse::<u64>().ok())
                    .unwrap_or(2097152) * 1024; // kB to bytes
            }
        }
    ...
    #[cfg(unix)]
    {
        ...
        if lines.len() >= 2 {
            ...
            if parts.len() >= 4 {
                ...
                return DiskSpace { total, used, available };
            }
        }
    }
...
async fn get_all_services() -> Vec<ServiceStatus> {
    ...
    let output = Command::new("docker")
        ...
        .unwrap_or_else(|_| {
            std::process::Output {
                stdout: b"NAMES\tSTATUS\tPORTS\ntest-service\tUp 2 hours\t0.0.0.0:8080->8080/tcp".to_vec(),
                stderr: vec![],
                status: create_success_exit_status(),
            }
        });
...
pub async fn update_ui_config(config: serde_json::Value) -> std::result::Result<(), Box<dyn std::error::Error>> {
