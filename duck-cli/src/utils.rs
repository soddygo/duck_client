...
pub fn copy_with_progress<R: Read, W: Write>(
    ...
    loop {
        ...
        if total_size > 100 * 1024 * 1024 { // 只对大于100MB的文件显示详细进度
            let percent = if total_size > 0 { (copied * 100) / total_size } else { 0 };
...
pub async fn extract_docker_service(zip_path: &std::path::Path) -> Result<()> {
    ...
    info!("   📊 总计需要解压: {} 个文件 (总大小: {:.1} GB)", total_files, total_size as f64 / 1024.0 / 1024.0 / 1024.0);
    ...
    for i in 0..archive.len() {
        ...
        if file.is_dir() {
            ...
            if file_size > 100 * 1024 * 1024 { // 大于100MB的文件
                info!("   📄 正在解压大文件: {} ({:.1} MB)", file.name(), file_size as f64 / 1024.0 / 1024.0);
            }
            ...
            if total_files > 0 {
                ...
                if percent != last_percent && (percent % 5 == 0 || extracted_files % 50 == 0) {
                    let size_percent = if total_size > 0 { (extracted_size * 100) / total_size } else { 0 };
                    info!("   📤 解压进度: {}% ({}/{} 文件, {:.1}% 大小)", 
    ...
    info!("   📤 解压完成: 100% ({}/{} 文件, {:.1} GB)", 
            extracted_files, total_files, extracted_size as f64 / 1024.0 / 1024.0 / 1024.0);
